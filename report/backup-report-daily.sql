CREATE table #temp_report( slno int identity(1,1),instance_name varchar(100), database_id int, database_name varchar(100), recovery_model varchar(50), state_desc varchar(50)
	, last_full_backup datetime, last_differential_backup datetime, last_log_backup datetime)
...
SELECT  @@SERVERNAME,database_id, name ,
    ...
    FROM    ( SELECT    db.database_id ,db.name ,
              ...
              FROM      master.sys.databases db
						...
						AND db.name NOT IN('maint','ReportServerTempDB','ReportServer','DBATools')
            ) AS Sourcetable 
        PIVOT 
...
--Declare variables for email
	...
	SET @TableHead = '<html><head>'+'</head><body><h3>Latest Backup History</h3><table border="1"><tr><th>Database Name</th><th>Recovery Model</th><th>State Desc</th>
				<th>Last Full Backup</th><th>Last Diff Backup</th><th>Last Log Backup</th></tr>'
...
IF EXISTS( select * from #temp_report where last_full_backup < GETDATE()-7 OR last_differential_backup < dateadd(hour,-24,getdate()) 
		OR last_log_backup < dateadd(hour,-4,getdate()) 
OR recovery_model <> 'FULL' and database_id > 5 and database_name <> 'maint')
BEGIN
	--select 1
	SET @TableHead2 = '</table><h3><span style="color:red;">Review below databases</span></h3><table border="1"><tr><th>Database Name</th><th>Recovery Model</th>
		<th>State Desc</th><th>Last Full Backup</th><th>Last Diff Backup</th><th>Last Log Backup</th></tr>'
